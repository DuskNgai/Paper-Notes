...
4. What is a "zero-loss" peak?
   ...
   > A peak that no energy loss in energy-amount graph.
...
5. What new capabilities do direct detectors provide?
   ...
   > Real-time and each burst in the image can be viewed as an single electron hit.
...
2. Why are so many different types of pumps needed?
   ...
   > They work under different range of air pressure and used one after one.

3. What is a "backing" pump?

   > Mechanical, Oil diffusion, Ion getter pump are working one after one.
...
1. What does a two-dimensional sine wave look like?
   ...
   > Similar to one-dimensional sine wave with one dimension extended without changing amplification.
